...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def _validate_credentials(self, credentials: CredentialsConfiguration):
        ...
        if (len(bearer_token) != 32):
            ...
            raise errors.PlunginError("The bearer_token must be 32 characters long")
    ...
    def _perform(self, validation_name, validation):
        self.ipv64.add_txt_record(validation_name, validation)

    def _cleanup(self, validation_name, validation):
...
class ipv64_client():
    ...
    def add_txt_record(self, domain: str, content: str):
        ...
        if (response.json()["info"] == "Unauthorized"):
            ...
            raise Exception("The following error occured while adding the TXT-Record " + domain + " to the zone " + dns_zone + ": " + response.json()["info"])
        ...
        if (response.json()["info"] != "success"):
            ...
            raise errors.PlunginError("The following error occured while adding the TXT-Record " + domain + " to the zone " + dns_zone + ": " + response.json()["add_record"])
